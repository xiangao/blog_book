...
The same requirement, close to word for word, shows up in the Registered Report guidelines at PNAS Nexus, Royal Society Open Science, and most other journals running the format — it traces back to a shared template from the Center for Open Science. There's also a power requirement attached that people don't always expect: proposals should be powered to detect *the smallest effect that is plausible and of theoretical value*, at 0.9 power or higher for frequentist plans, and pilot data alone is explicitly called "unlikely to form an acceptable basis" for choosing that effect size. I'll come back to why that specific phrasing matters.
...
3. **Small telescopes** (Simonsohn, 2015) — anchor $\Delta$ to some fraction (Simonsohn used 33%) of the effect size a comparable prior study's sample size could have reliably detected. This one has an actual formula behind it, but only works if a genuinely comparable prior study exists.
