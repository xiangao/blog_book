...
This is basically OLS on de-meaned $Y$ and $X$.  Note that the intercept $\alpha$ and the individual effect $v_i$ both drop out after de-meaning by unit.
...
Correlated random effect (CRE) can be done by running a random effect model of $y_{it}$ on $X_{it}$, $\bar X_i$, $w_i$ (time-invariant covariates) and a constant.
