...
date: "2024-08-08"
execute:
  freeze: true   # radiant is not installable on current R; always use the frozen output
---
...
# NOTE: this URL now returns HTTP 401 (the host requires authentication), so this
# chunk cannot be re-executed as written -- the chapter renders from a frozen
# result. Substitute a local copy of the ice-cream data to run it again.
...
The data are 150 rows: 15 respondents each rating 10 ice-cream profiles, in long
format after pivoting.
...
attribute2 <- "Packaging; Homemade waffle; Cone; Pint"
...
::: {.callout-warning}
**Dependency note.** This chapter depends on the `radiant` package, whose plotting functions can fail to install or run on current R versions. If `radiant` is unavailable in your environment, the design-of-experiment and plotting chunks below will not execute; the `doe` output and figures are kept from a prior successful render.
:::
...
Note also that the off-diagonal correlations among Flavor, Package and the other
attributes are of order $10^{-16}$ — numerically zero. Only the
Light-Organic pair is confounded; the rest of the design is orthogonal.
...
partworths_lm <- lm(rating ~ Flavor + Packaging + Light + Organic, data = icecream)
summary(partworths_lm)

# Each of the 15 respondents rates 10 profiles, so observations are clustered
# by respondent; plain OLS standard errors above assume independence and
# understate the true uncertainty. Cluster at the respondent level instead.
coeftest(partworths_lm, vcov = vcovCL(partworths_lm, cluster = icecream$respondent))
