...
Revision history for *Topics on econometrics and causal inference*: review passes,
technical audits, and corrections. Entries are in the order they were appended, which is
not strictly chronological — each carries its own date.

One caveat for reading the older entries. On 2026-09-03 six chapters were replaced by
shorter rewrites (commit `d6bd048`): `gwg`, `likert-scale-variance`, `equivalence-testing`,
...
> **2026-09-08:** Acted on a Codex review (`REVIEW_SUGGESTIONS_20260908.md`). The September
> rewrites had removed methodological qualifications that the earlier chapters carried, and
> two chapters were left asserting more than their own analysis supports. In `gwg.qmd`:
> qualified the "explained component is selection bias" claim with the assumptions it needs;
> added the Holland manipulability caveat on gender-as-treatment and the full-time-worker
> selection caveat; replaced the "Interpretation Trade-Off" list with the ladder-of-controlled-
> direct-effects table, since the old text simultaneously claimed the −11.3% → −7.5% move
> measured occupational sorting and that occupation is a collider; attached the no-unmeasured-
> mediator–outcome-confounding condition to the CDE label; and rewrote the AIPW section, which
> had described a functional-form robustness check as machine-learning confirmation. In
> `likert-scale-variance.qmd`: restored the common-thresholds identifying assumption (the
> simulation shares one `cuts` vector across groups, so the ordinal model succeeds partly by
> construction) and softened the claims that the model "separates" dispersion from boundary
> artifacts and that pooled R "removes" boundary effects. Housekeeping: HTML page TOC turned
> on (`toc: right`, depth 3); reading paths added to the preface; this changelog split out of
> the README. Prose only — no chunk touched, no estimate changed. Two review items were
