...
So our advice for a conditional Poisson model is that we should not
...
"incidental parameter" problem.  Again, we should just focus on the
...
Therefore, we can use margins command to calculate effects on the
logged odds, which will be "predict(xb)" option.  Note that `xb` here
