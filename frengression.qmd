...
Note that the training chunks here carry `cache: false` and torch's random
number generator is not fixed by R's `set.seed()`, so this particular figure
moves by a percentage point or two between renders. The cached comparison table
below is the number to quote.
