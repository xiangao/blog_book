...
The gap is not academic.  Using `nlswork`, which is unbalanced (persons contribute between 1
...
- **keep the other dimensions as dummies**, since they are cheap;
- take the means over the chosen dimension of *everything else in the model*, the retained dummies
  included.

That last clause is the one people miss, and it is the same clause that fixes the unbalanced two-way case
above.  Call this construction **B**, and call the three-sets-of-means guess **A**.
...
So **A is not wrong so much as conditional**: it is exact when the factors are mutually orthogonal, and
it drifts otherwise.  A full factorial design gives orthogonality, but so does a Latin square, which
...
orthogonal the design has drifted.
