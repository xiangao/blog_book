...
1. Consistency.  The observed outcome under the treatment actually received equals the corresponding potential outcome, and the shifted intervention is well defined --- no hidden versions of it.

2. Sequential unconfoundedness.  There are no unmeasured confounders of treatment at any time point given the history.  The stronger *sequential randomization* condition conditions on the full history including past natural values of treatment; some estimators need that version and others do not, so it is worth knowing which one a given result assumes.

3. Positivity, in the form the shift requires.  Roughly: if a unit with history $h_t$ can receive $a_t$, then the shifted value $d(a_t, h_t)$ must also have support given $h_t$.  This is where modified treatment policies earn their keep.  A static always-treat or never-treat rule can demand support the data does not have, while a bounded shift of the observed treatment stays inside it.

4. For continuous treatments, a smoothness condition.  The influence-function estimators below assume the shift function is piecewise smooth and invertible in $a_t$, which is what lets the density of the shifted treatment be written in terms of the observed one.

The always-treat and never-treat demonstration below is, on that reading, an unusual lead example for this framework: it is the easiest case to follow, and also the case whose positivity is most likely to fail.  A bounded continuous shift is the canonical LMTP application.  Díaz et al., <https://arxiv.org/abs/2006.01366>, state the assumptions precisely.
...
The always-treat regime gives $E[Y^d] = 2.029$, never-treat gives 0.129, and
...
show_lmtp(lmtp_contrast(fit_dtr, fit_MR, ref = fit_CD))
