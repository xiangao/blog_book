...
Instead, Wooldridge suggests using CRE probit.  Basically adding Mundlak device (group means of $X$'s) to the model, and then use pooled probit.  Note that we do not use RE probit or logit, since that would need conditional independence too.  Lin and Wooldridge (2019, sec. 5.2) put this plainly: "None of random effects probit, RE logit, RE Tobit, RE Poisson, and so on has robustness properties in the presence of serial correlation of the innovations."  Pooled methods with cluster-robust inference are robust to serial dependence of arbitrary form, which is the reason to prefer them here.
