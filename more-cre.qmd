...
time-varying regressors, and nothing else.  For a *two-way* model the time dimension has to be
handled as well --- either by adding the time means of the regressors, or by keeping a full set of
time dummies.  The example below does the latter (unit means plus `factor(year)`), which is why it
reproduces the *two-way* fixed effect estimate.  Unit means alone would reproduce only the
*one-way* estimate.

**The two-way equivalence requires a balanced panel.**  The one-way result holds whatever the panel
looks like, because $\sum_t (x_{it} - \bar x_i) = 0$ within each unit by construction.  The two-way
result additionally needs the unit and time factors to be orthogonal, which a balanced panel
delivers and an unbalanced one does not.  The `castle` data used below is balanced, so the
equivalence holds exactly.  On an unbalanced panel it does not, and the discrepancy can be
substantial --- see [the qualification in the Mundlak chapter](mundlak-device.qmd), where the
coefficient on `age` in `nlswork` differs by more than a factor of two.  What restores it there is
adding the unit means of the time dummies.
