...
This estimator has a published treatment worth knowing about.  Lin and Wooldridge (2019), "Testing and correcting for endogeneity in nonlinear unobserved effects models," treats exactly this setup: their **Procedure 3** is a reduced-form residual added to an FE Poisson, with a robust Wald test on the residual's coefficient serving as a test of endogeneity.  Several points below come from it.
...
Two honest caveats.  The construction is introduced as "a simple example", and is immediately followed
...
future research."  So this is the construction to reach for, not a settled result.  If you are worried
...
Given that, when the endogenous variable is binary I would report a linear first stage alongside the
logit or probit one.  The linear version rests on the exact result in point 2 above and drops no
observations; the nonlinear version is better specified for a binary variable but rests on the unsettled
argument here.  Agreement between the two is the useful evidence.
