...
first stage needs the treatment set out in the [Mundlak chapter](mundlak-device.qmd): take means over the
...
The CRE first stage is better but not unbiased: in the same simulation it ran roughly ten per cent low at
...
* 2. control function with a CRE (Mundlak) logit first stage.
*    For a logit the generalized residual is just d - phat.
...
di "CF, CRE logit first stage       = " %8.6f b_cre
